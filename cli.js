...
/** Display DOT account balance */
async function printETHBalance({ address, name }) {
  console.log(`${name} DOT balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'DOT' && refund !== '0') {
    throw new Error('The DOT purchase is supposted to be 0 for DOT withdrawals')
  }
  refund = toWei(refund)
  if (relayerURL) {
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    try {
      if (web3 == undefined) {
        ...
        contractJson = await (await fetch('build/contracts/DOTSmashnado.json')).json()
    ...
    if (web3 === undefined) {
      ...
      contractJson = require('./build/contracts/DOTSmashnado.json')
  ...
  erc20 = currency !== 'dot' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  if (inBrowser) {
    const instance = { currency: 'dot', amount: '0.1' }
    //await init(instance)
